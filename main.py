...
system_rules_short = (
    "فقط متن قرارداد رسمی فارسی تولید کن؛ بدون توضیح خارج از قرارداد. "
    "ماده‌بندی شفاف با «ماده ۱، ماده ۲…» و در صورت نیاز زیر‌بند «۱-۱، ۱-۲…». "
    "اعداد با ارقام فارسی ۰۱۲۳۴۵۶۷۸۹؛ مبالغ مهم هم رقم و هم حروف (مثلاً ۵۰۰٬۰۰۰٬۰۰۰ (پانصد میلیون) ریال). "
    "تاریخ‌ها به قالب ۱۴۰۴/۰۷/۱۸ و در صورت اهمیت، همراه حروف. "
    "اگر بخشی از ورودی خالی است، بند استاندارد محافظه‌کارانه بساز و ابتدای آن «[پیشنهادی]» بگذار. "
    "از تضمین‌های مطلق/غیرواقعی و ارجاع به حقوق خارجی پرهیز کن؛ قانون حاکم ایران مفروض است. "
    "بخش‌های ضروری: عنوان و مشخصات طرفین، تعاریف (در صورت نیاز)، موضوع و دامنه، مدت/برنامه، مبلغ/روش پرداخت/کسورات، "
    "تعهدات و کیفیت (در صورت لزوم KPI/SLA)، تحویل و پذیرش/انتقال ریسک، تضمین و رفع عیوب، محرمانگی، مالکیت فکری، "
    "تغییرات/الحاقات، قوه قاهره، وجه‌التزام/خسارات، فسخ و خاتمه، حل اختلاف (مذاکره→میانجی‌گری(اختیاری)→داوری(در صورت ذکر)→مرجع قضایی)، "
    "ابلاغ و نشانی قراردادی، قابل تفکیک بودن مواد، قانون حاکم و زبان، پیوست‌ها/ضمائم."
)

# --- AI Contract: type-specific tweaks
TYPE_TWEAKS = {
    "خدمات": (
        "ریزنکات خدمات: دامنه کار (SOW) دقیق و قابل سنجش؛ SLA/KPI با آستانه پذیرش؛ "
        "زمان‌بندی اجرا و دسترسی‌های موردنیاز؛ مالکیت نتایج؛ محرمانگی/عدم رقابت (در صورت اقتضا)؛ "
        "پرداخت مرحله‌ای/ماهانه و پیش‌نیاز صورتحساب؛ معیار پذیرش و فرآیند رفع نقص."
    ),
    "خرید و فروش": (
        "ریزنکات خرید و فروش: مشخصات فنی/استاندارد کالا؛ مقدار/بسته‌بندی؛ محل و شیوه تحویل؛ انتقال ریسک/مالکیت؛ "
        "در صورت لزوم شروط اینکوترمز؛ بازرسی/پذیرش؛ مرجوعی عیوب ظاهری/پنهان؛ مالیات/عوارض؛ "
        "جریمه تأخیر تحویل و دیرکرد پرداخت."
    ),
    "اجاره": (
        "ریزنکات اجاره: توصیف عین مستأجره؛ مدت و اجاره‌بها/شیوه پرداخت؛ ودیعه و شرایط استرداد؛ "
        "استفاده مجاز/ممنوع، نگهداری/تعمیرات؛ مسئولیت خسارات؛ واگذاری به غیر؛ تحویل و عودت؛ "
        "فسخ ناشی از عدم پرداخت یا تخلف اساسی."
    ),
    "مشارکت": (
        "ریزنکات مشارکت: آورده‌های نقدی/غیرنقدی و نحوه ارزیابی؛ نسبت سود/زیان؛ حساب مشترک و امضاداران؛ "
        "سازوکار تصمیم‌گیری و حق وتو؛ هزینه‌های قابل‌قبول و شفافیت مالی/گزارش‌دهی؛ حسابرسی دوره‌ای؛ "
        "ترک شراکت/انتقال سهم‌الشرکه؛ تضمین انجام تعهدات طرفین."
    ),
}

def pick_type_tweak(contract_type: str) -> str:
    ct = (contract_type or "").strip()
    for key in TYPE_TWEAKS.keys():
        if key in ct:
            return TYPE_TWEAKS[key]
    # تطبیق‌های رایج
    if any(k in ct for k in ["ارائه خدمات","سرویس"]): return TYPE_TWEAKS["خدمات"]
    if any(k in ct for k in ["خرید","فروش","بیع"]):   return TYPE_TWEAKS["خرید و فروش"]
    if any(k in ct for k in ["اجاره","استیجاری"]):    return TYPE_TWEAKS["اجاره"]
    if any(k in ct for k in ["شراکت","شریک"]):        return TYPE_TWEAKS["مشارکت"]
    return ""

def build_contract_prompt_flexible(نوع, طرف_اول, طرف_دوم, موضوع, مبلغ_و_پرداخت, مدت, تعهدات, وجه_التزام, فسخ, حل_اختلاف, محل_تاریخ_شهود, missing_fields):
    # ⬇️ ریزتنظیم نوع قرارداد
    tweak = pick_type_tweak(نوع)
    sys_rules = system_rules_short + (("\n" + tweak) if tweak else "")
    ...
    بخش‌های ناقص: {", ".join(missing_fields) if missing_fields else "—"}
    """
    return (
        "SYSTEM:\n" + sys_rules + "\n\n" +
        "USER:\n" + user_facts + "\n\n" +
        "خروجی را فقط به صورت متن کامل قرارداد تولید کن. بندهای تکمیلی را با «[پیشنهادی]» علامت‌گذاری کن."
    )
...
def finish_contract_generation(chat_id, data, context):
    ...
    prompt = build_contract_prompt_flexible(
        ...
        missing_fields=missing,  # ⬅️ این خط
    )
    ...
    try:
        ...
        if data.get("ai_mode") == "name_suggestion":
            ...
            if step == 2:
                ...
                system_prompt = (
                    "نام‌های پیشنهادی باید ریشه فارسی داشته باشد و کاملاً معنادار و حرفه‌ای باشند، "
                    "دست‌کم دارای یک اسم خاص باشد، از ترکیب کلمه‌ی کاربر با مفاهیم مرتبط با حوزه‌ی فعالیت، "
                    "از این کلمات استفاده نکن: (ملی، ایران، ملت، کشور، انتظام، نظام، نفت، پتروشیمی، دادگستر، بنیاد، سازمان، مرکز، "
                    "بهزیست، بانک، بسیج، جهاد، آسانسور، آژانس، ایثار، ایثارگران، شاهد، شهید، آزاده، جانبازان، تکنو، فامیلی، فیلتر، "
                    "نیک، باور، میکروبرد، تست، استار، تک، مدرن، پیک، امنیت، کارواش، فانتزی، شهروند، اقتصاد، میهن، جوانان، ایمان، "
                    "دکوراسیون، هدف، سپاه، تکنیک، دیتا، تک نو، تکنولوژی، اورست، مونتاژ، ونوس، دفتر، ایتال، بیو، کنترل، متریک، مترولوژی، وب). "
                    "برخی کلمات در نام شرکت‌ها جزو سیلاب محسوب نمی‌شوند مثل:صفت‌هایی نظیرِ برتر، خوب، نیک  و ... نام شهرها، رنگ‌ها و اعداد مانندِ تهران، فیروزه‌ای، چهل و ... کلماتی که بیانگر موضوع فعالیت هستند، مثلِ ساختمانی، حقوقی، بازرگانی، صنعتی، خدماتی، توسعه، تجاری، مهندسی، مهندسی مشاور، فنی مهندسی، تولیدی، هرچند استفاده از این واژه‌ها در اسم شرکت مانعی ندارد "
                    "اما جزئی از نام شرکت شمرده نمی‌شوند و در نام‌گذاری باید به این نکته توجه کرد که اسم شخصیت حقوقی، غیر از این کلمات، "
                    "شامل حداقل سه سیلاب (سه کلمه) باشد."
                )
